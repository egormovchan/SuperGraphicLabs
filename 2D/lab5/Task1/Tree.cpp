...
struct LSystem {
    ...
    void draw() {
        ...
        scale *= 1.3;
        std::stack<std::pair<point, float>> st;
        point direction = radiusVector;
        ImColor brown(108, 44, 28, 255);
        ImColor green(0, 128, 0, 255);
        ImColor color(128, 64, 48, 255);
        point from = point(minX - 0, minY - 250) * -1;
        int i = 0;
        for (auto c : to_draw) {
            float angle_to_rotate = 0;
            switch (c) {
            case '+':
                angle_to_rotate = rand() % (int)angle;
                if (current_angle + angle_to_rotate > 50)
                    angle_to_rotate = 50 - current_angle;
                current_angle += angle_to_rotate;
                direction.rotate(angle_to_rotate);
                break;
            case '-':
                angle_to_rotate = -(rand() % (int)angle);
                if (current_angle + angle_to_rotate < -65)
                    angle_to_rotate = -65 - current_angle;
                current_angle += angle_to_rotate;
                direction.rotate(angle_to_rotate);
                break;
            case '[':
                scale -= 1;
                color = GetGradientColor(brown, green, times, i++);
                st.push({ from, current_angle });
                break;
            case ']':
                scale += 1;
                color = GetGradientColor(brown, green, times, --i);
                from = st.top().first;
                //current_angle = st.top().second;
                st.pop();
                break;
            case '@':
            ...
            case 'F':
                ...
                ImGui::GetForegroundDrawList()->
                    AddLine(ImVec2(from.x, from.y), ImVec2(to.x, to.y), color, 1.5 * times - 1.3 * i);
                std::swap(from, to);
                break;
            }
        }
