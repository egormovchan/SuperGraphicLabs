...
void set_pixel(const point& p, ImColor& c) {
    ...
    if (p.z > z_buffer[static_cast<int>(p.x) * SCREENW + static_cast<int>(p.y)]) {
        ...
        screen[static_cast<int>(p.x) * SCREENW + static_cast<int>(p.y)] = c;
    }
}

void set_screen(int val) {
    std::fill(screen, screen + SCREENW * SCREENH, ImColor(255, 255, 255));
}
...
unsigned char* textureData = LoadTextureFromFile("text.jpg", texWidth, texHeight, texChannels);

void DrawLineBresenham(const point& p0, const point& p1,
    const point& pt0, const point& pt1) {
    int x0 = p0.x, y0 = p0.y, z0 = p0.z;
    ...
    int errX = maxDelta / 2, errY = maxDelta / 2, errZ = maxDelta / 2;
...
double InterpoalteFactor(float y, const point& p1, const point& p2) {
    return (abs(p2.y - p1.y) < 0.01) ? 0.0 : (y - p1.y) / (p2.y - p1.y);
}

point InterpolateVertexY(float factor, const point& p1, const point& p2) {
...
void DrawTriangle(const point& p1, const point& p2, const point& p3
    ...
    for (float y = top.y; y <= mid.y - 0.001; ++y) {
        ...
        DrawLineBresenham(left, right, text_left, text_right);
    }

    for (float y = mid.y; y <= bot.y - 0.001; ++y) {
        ...
        void draw() const {
            if (use_z_buffer) {
                ...
                while (nit != vertices.end()) {
                    DrawTriangle(**vertices.begin(), **it, **nit
                        , *text_indices[ti1], *text_indices[ti2], *text_indices[ti3], shade);
                    ti2++;
                    ti3++;
                    ++it;
                    ++nit;
...
public:
    ...
    void tie_text_vertex_to_face(uint text_vertex_index, uint face_index) {
        pol->faces[face_index].text_indices.push_back(&text_vertices[text_vertex_index]);
    }
